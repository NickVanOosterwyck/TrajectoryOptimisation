...
for k=0:n-1
    i=k+1;
    C(i) = zi(i+1)/6/hi(i)*(t-ti(i))^3 + zi(i)/6/hi(i)*(ti(i+1)-t)^3 + ...
        ((thi(i+1)/hi(i))-(hi(i)/6*zi(i+1)))*(t-ti(i)) + ...
        ((thi(i)/hi(i))-(hi(i)/6*zi(i)))*(ti(i+1)-t);
end
...
% for k=0:n-1
%     i=k+1;
%     Cd2b(i) = (zi(i+1)-zi(i))/hi(i)*(t-ti(i)) + zi(i);
% end
...
Aeq(n,1) = -1/(t0 - ti(2));
Aeq(n,n) = t0/6 - ti(2)/6;
Aeq(n+1,n-1) = -1/(ti(n) - tn);
Aeq(n+1,2*n-2) = ti(n)/6 - tn/6;
